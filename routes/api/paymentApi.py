...
paymentApi = Blueprint("paymentApi", __name__)
...
@paymentApi.route("/api/payment/history-inq", methods=['POST', 'GET'])
def history():
    ...
    print("end : " + end)
    result_data = {
        "recordsFiltered": len(payment_data),  # 페이징 처리용 total
        "data": payment_data,
        "totalData": {
            "sum": "1,400,000",
            "cancelSum": "800,000",
...
@paymentApi.route("/api/payment/cancel-history-inq", methods=['POST', 'GET'])
def cancel_history():
    # url = 'http://kpcApiServer/'
    # data = request.get_json()
    # headers = {'Content-Type' : 'application/json'}
